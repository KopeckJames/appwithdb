...
class CoreDataMigration {
    ...
    private init() {
        print("CoreDataMigration initialized")
    }
    
    /// Performs necessary migrations for the Core Data model
    func performMigrations() {
        // Update the Meal entity with new attributes for meal analysis
        updateMealEntity()
    }
    
    /// Updates the Meal entity with new attributes for meal analysis
    private func updateMealEntity() {
        ...
        // Create a new Meal entity description if it doesn't exist
        let entityName = "Meal"
        var entityDescription = NSEntityDescription.entity(forEntityName: entityName, in: context)
        
        if entityDescription == nil {
            // Create a new entity description
            entityDescription = NSEntityDescription()
            entityDescription?.name = entityName
            entityDescription?.managedObjectClassName = "Meal"
        }
        
        guard let entityDescription = entityDescription else {
            print("Failed to get or create entity description for Meal")
            return
        }
    ...
    private func needsUpdate(context: NSManagedObjectContext) -> Bool {
        guard let entityDescription = NSEntityDescription.entity(forEntityName: "Meal", in: context) else {
            return true // Entity doesn't exist, so we need to update
        }
